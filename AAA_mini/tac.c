...
TAC *declare_var_char(char *name)
{
	SYM*t = mk_var(name);
	t->char_flag = 1;
	return mk_tac(TAC_VAR,t,NULL,NULL);
}
...
TAC *declare_para_address(char *name)
{
	SYM*t=mk_var(name);
	t->address_flag = 1;
	return mk_tac(TAC_FORMAL,t,NULL,NULL);
}
...
EXP *do_bin( int binop, EXP *exp1, EXP *exp2)
{
	...
	int boolA = ((exp1->ret->type==SYM_INT) || (exp1->ret->type==SYM_CHAR));
    int boolB = ((exp2->ret->type==SYM_INT) || (exp2->ret->type==SYM_CHAR));
    int boolChar = ((exp1->ret->type==SYM_CHAR) || (exp2->ret->type==SYM_CHAR));
...
TAC *do_for(TAC *one, EXP *exp, TAC *two,TAC *stmt) 
{
	TAC *label=mk_tac(TAC_LABEL, mk_label(mk_lstr(next_label++)), NULL, NULL);
    TAC *code=mk_tac(TAC_GOTO, label->a, NULL, NULL);
	two = join_tac(stmt, two);
	code->prev = two;
    TAC *A = join_tac(label, do_if(exp, code));

    return join_tac(one, A);
}
